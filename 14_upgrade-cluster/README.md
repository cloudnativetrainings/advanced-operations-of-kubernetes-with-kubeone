...
```bash
# verify your current version 1.36.3
kubectl version
```
...
```bash
# open a new terminal
# => with this we verify the applications are up and running during the upgrade process
...
sed -i "s/kubelet: 1.36.3$/kubelet: 1.36.4/g" /training/md-europe-west3-b.yaml
sed -i "s/kubelet: 1.36.3$/kubelet: 1.36.4/g" /training/md-europe-west3-c.yaml
```

```bash
# open a new terminal
# => with this we watch the machinecontroller upgrading the worker nodes
watch -n 1 kubectl -n kube-system get machinedeployment,machineset,machine,nodes
```
