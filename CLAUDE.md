...
2. `01_install-k1` — install pinned KubeOne `1.13.3` (later upgraded to `1.13.4` in lab 15).
...
10. `09_helm-releases` — add `helmReleases:` block to `kubeone.yaml` for `ingress-nginx` + `cert-manager`, apply `cluster-issuer.yaml`, set up the DNS entry for the domain.
