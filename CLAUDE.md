...
It asserts `/root/.trainingrc` exists and is sourced from `/root/.zshrc` (the shell is zsh + oh-my-zsh + powerlevel10k), that the CLI tools respond, that `GCP_PROJECT`, `TRAINEE_NAME`, `DOMAIN`, `DNS_ZONE_NAME`, `K8S_VERSION` and `TF_VERSION` are set, and that `/training/.secrets/` holds `gcp`, `gcp.pub` and `gcp-service-account.json`.
...
- **The image is amd64-only.** `--platform linux/amd64` is pinned in both `build` and `run`, so it runs under Rosetta/QEMU on arm64 hosts. A native arm64 build fails: five hardcoded `amd64` download URLs, plus GNU tar 1.35 hitting an unimplemented `openat2` under Rosetta, which breaks the helm and velero extraction steps. See **`container-image/build.md`** for the measured details and the `TARGETARCH` migration path.
...
13. `14_upgrade-cluster` — bump `versions.kubernetes` in `kubeone.yaml`, `kubeone apply` for the control plane, edit `kubelet:` in the MD manifests for workers.
...
- **`kubeone.yaml`** — root KubeOne manifest (`kubeone.k8c.io/v1beta2`, `KubeOneCluster`). Kubernetes `1.36.3`, `cloudProvider.gce`, `external: true` CCM. It **already contains** the `helmReleases:` block for `ingress-nginx` `4.12.2` and `cert-manager` `v1.17.2`. Most `kubeone` commands either run from `/training/` (auto-detect) or take `-m /training/kubeone.yaml`.
...
- **`09_helm-releases/README.md` tells trainees to add the `helmReleases:` block to `kubeone.yaml`, but the checked-in `kubeone.yaml` already contains it.**
- **`09_helm-releases/README.md` calls `nslookup $DOMAIN`, but no DNS tooling is installed** — the dockerfile pulls neither `bind9-dnsutils` nor `dnsutils`, so neither `nslookup` nor `dig` exists in the image. The previously published `1.0.0` image had them.
- **`02_terraform/README.md` links to `./tf_infra/README.md`**, which resolves to `02_terraform/tf_infra/README.md`. The correct relative path is `../tf_infra/README.md`.
- **`15_upgrade-k1/README.md` copies from `kubeone_${NEW_KUBEONE_VERSION}_linux_amd64/`**, but `get.kubeone.io` detects the architecture, so the path is wrong on arm64. `14_upgrade-cluster/README.md` likewise hardcodes an `amd64` kubectl download.
